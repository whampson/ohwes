...
#include <kernel/pool2.h>

#define CHUNK_MAGIC         'nuhc'

#define pool_valid(p)   ((p) != INVALID_POOL2 && (p)->magic == POOL2_MAGIC)

// memory 'chunk' info for keeping track of pool item
struct chunk {
    uint32_t magic;     // identifier for chunk type
    pool2_t *pool;      // pool this item belongs to
...
void lazy_init_pools(void)
{
    ...
    size_t master_pool_size = align(num_pools * sizeof(struct pool2), 4);
    g_pools->order = get_order(master_pool_size);

    g_pools->pools = (struct pool2 *) alloc_pages(ALLOC_ZERO, g_pools->order);
...
pool2_t * pool2_create(const char *name, size_t size, size_t capacity, int flags)
{
    ...
    p = list_item(g_pools->free_list.next, struct pool2, list);
    assert(p->alloc == NULL);
...
void pool2_destroy(pool2_t *pool)
{
    ...
    for (list_iterator(&g_pools->list, n)) {    // TODO: consider list_iterator(n, &g_pools->list), implicitly declare n type
        p = list_item(n, struct pool2, list);
        if (p == pool) {
            break;
        }
    }
...
void * pool2_alloc(pool2_t *pool, int flags)
{
    ...
    assert(chunk->pool == INVALID_POOL2);
