...
//     // ps2_init();
//     if (!ps2_test()) panic("ps2ctl: self-test failed!");
...
readsc:
    ...
    switch (sc)
    {
        case 0xFA:
            if (num_ack == 0) {
                kprint("ps2kbd: got unexpected ACK\n");
            }
            --num_ack;
            goto done;
        case 0xFE:
            kprint("ps2kbd: please resend command\n");
            goto done;
        case 0xFF:
        case 0x00:
            kprint("ps2kbd: error %02X", sc);
            goto done;
    }
    ...
    if ((vk & 0x7F) == 0) {
        kprint("ps2kbd: got unrecognized scancode: %02X\n", sc);
        goto done;
    }
